...
import { Globe, Play, Square, Loader2, Link as LinkIcon, RefreshCw, Gamepad2 } from "lucide-react";
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className={`bg-muted-subtle border border-border-subtle rounded-xl p-6 shadow-sm ${serverRuntimeType === 'local' ? 'opacity-50 pointer-events-none' : ''}`}>
          {serverData?.bedrockPort && (
            <div className="mb-6 p-4 bg-theme-500/5 border border-theme-500/20 rounded-lg flex items-start gap-3">
              <Gamepad2 className="w-5 h-5 text-theme-500 shrink-0 mt-0.5" />
              <div className="text-sm">
                <h3 className="font-semibold text-foreground mb-1">Bedrock Support Enabled</h3>
                <p className="text-muted-foreground text-[13px]">
                  This server exposes Geyser on UDP port <span className="font-mono text-foreground">{serverData.bedrockPort}</span>.
                  After claiming the agent on playit.gg, add the GeyserMC plugin to your Java server, then in the playit.gg
                  dashboard create an additional <span className="font-mono text-foreground">UDP tunnel → local port {serverData.bedrockPort}</span>.
                  Bedrock players use <span className="font-mono text-foreground">ip:bedrock_tunnel_port</span> to connect.
                </p>
                <ol className="mt-2 text-muted-foreground text-[12px] list-decimal list-inside space-y-1">
                  <li>Install the GeyserMC + Floodgate plugins on the Java server</li>
                  <li>Generate the tunnel and claim the agent above</li>
                  <li>In playit.gg, create a UDP tunnel pointing to local port {serverData.bedrockPort}</li>
                </ol>
              </div>
            </div>
          )}
