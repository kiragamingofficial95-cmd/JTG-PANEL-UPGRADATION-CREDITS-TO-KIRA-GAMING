...
export default function CreateServer() {
  ...
  const [state, setState] = useState({
    name: '', desc: '', ram: 4, cpu: 150, disk: 10, ip: '', port: 25565, bedrockPort: 19132, runtimeType: 'docker', 
  ...
  const launch = async () => {
    ...
    try {
      const payload = {
        ...
        port: state.port,
        bedrockPort: state.bedrockPort || null,
        ipAlias: state.ip,
        type: state.software,
            ...
            <div className={`${dir === 'forward' ? 'anim-forward' : 'anim-back'}`}>
              ...
              {currentStep === 2 && (
                <div className="step-content">
                  ...
                  <label className="flex items-center gap-2 text-sm text-[#8f8f8f] mb-2.5">
                    <Network className="w-4 h-4" /> Bedrock Port (Geyser)
                  </label>
                  <input 
                    type="number" className="inp font-mono" placeholder="19132" 
                    value={state.bedrockPort || ''} onChange={e => updateState('bedrockPort', e.target.value ? Number(e.target.value) : '')}
                  />
                  <p className="text-[11px] text-[#4c4c4c] mt-2 mb-8 font-mono">
                    Optional port for Bedrock/Windows 10+ players via Geyser. Leave empty to disable.
                  </p>
