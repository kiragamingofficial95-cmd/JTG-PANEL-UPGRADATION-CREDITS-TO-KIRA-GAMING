...
import { Server, LayoutDashboard, Plus, LogOut, X, Settings, Key, User, Activity, Box, Search, Bell, Menu, Boxes, Sparkles } from "lucide-react";
...
export function Sidebar({ onClose, isCollapsed, toggleCollapse }: { onClose?: () => void, isCollapsed?: boolean, toggleCollapse?: () => void }) {
  ...
  useEffect(() => {
    if (!user) return;
    axios.get("/api/blueprint/runtime/navigation")
      .then(res => {
        if (res.data && Array.isArray(res.data.navItems)) {
          setExtensionNavItems(res.data.navItems);
        }
      })
      .catch(() => {});
  ...
  // Dynamic extension navigation links
  if (extensionNavItems.length > 0) {
    for (const extNav of extensionNavItems) {
      if (extNav.section === "admin" && user?.role !== "admin" && user?.role !== "owner") {
        continue;
      }
      links.push({
        name: extNav.title || extNav.name,
        path: extNav.path,
        icon: <Boxes size={20} className="text-theme-400" />
      });
    }
  }
